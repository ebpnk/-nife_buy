...
namespace KnifeShopPlugin
{
    public class KnifeShop : BasePlugin
    {
        ...
        public override void Load(bool hotReload)
        {
            base.Load(hotReload);
            LoadConfig();
            AddCommand("buykf", "Buy a knife", CommandBuyKnife);
        ...
        private void AttemptToPurchaseKnife(CCSPlayerController player, string knifeCommand, KnifeConfig knifeConfig)
        {
            if (player.InGameMoneyServices.Account >= knifeConfig.Price)
            {
                ...
                player.PrintToChat($"You bought {knifeConfig.Name} for {knifeConfig.Price}$, which will be available for {knifeConfig.Duration} seconds.");
            ...
            {
                player.PrintToChat("You don't have enough money to buy this knife.");
            }
        }
        ...
        private void SetTimerToRemoveKnife(ulong steamId, int duration)
        {
            new Timer(duration, () =>
            {
                ...
                if (player != null)
                {
                    player.PrintToChat("Your knife has been confiscated.");
                }
            });
        }
